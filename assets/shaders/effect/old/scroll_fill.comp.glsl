...
// Deterministic spiral / ring search for a valid neighbor in the CURRENT frame
// that belongs to the same object id and has already been written (noise.g >= 0.5).
bool findNeighborAccSpiral(ivec2 px, int id, ivec2 size, int maxR, out vec2 accOut) {
  //accOut = imageLoad(uCurrAccTex, px + ivec2(-50, 0)).xy;
  //return true; // DEBUG
...
void main() {
  ...
  if (noise.g < 0.5) {
    ...
    float noiseVal = step(0.5, rng(uvec2(px))); // TMP
    imageStore(uCurrNoiseTex, px, vec4(noiseVal, 0.25, 0, 0)); // TMP
    ...
    if (currId >= 0) {
      ...
      if (prevId == currId) {
        ...
        vec2 neighAcc;
        const int MAX_R = 16; // tweak; 8..32 typical
