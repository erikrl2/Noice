...
layout(r32ui, binding = 5) uniform uimage2D uNeedsJfaFlag;
layout(r8ui, binding = 6) uniform writeonly uimage2D uNeedsJfaMask;
...
void main() {
  ...
  if (noise.g < 0.5) {
    ...
    if (currId >= 0) {
      ...
      if (prevId == currId) {
        // scroll holes: inherited acc
        acc = imageLoad(uPrevAccTex, px).xy;
      } else {
        // disocclusion holes: spiral fast path
        vec2 neighAcc;
        const int MAX_R = 32;
        if (findNeighborAccSpiral(px, currId, size, MAX_R, neighAcc)) {
          acc = neighAcc;
        } else {
          // mark for JFA fixup pass
          imageAtomicOr(uNeedsJfaFlag, ivec2(0, 0), 1u);
          imageStore(uNeedsJfaMask, px, uvec4(1, 0, 0, 0));
        }
      }
    }
